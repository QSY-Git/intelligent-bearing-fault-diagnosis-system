...
for _f in ["Microsoft YaHei","SimHei","KaiTi","SimSun"]:
    if _f in _af:
...
# 数据
# ═══════════════════════════════════════════════
from core.signal.loader import build_dataset, LABEL_MAP
from core.persistence.store import DiagnosticsStore

LABEL_DICT = {v[0]: v[1] for v in LABEL_MAP.values()}
X_all, y_all, _ = build_dataset(window_size=1024, step=512)
store = DiagnosticsStore(memory_size=5000, db_path=os.path.join(os.path.dirname(__file__),"diagnostics.db"))
stats = store.stats()
...
# 指标卡
mc = st.columns(5)
mc[0].markdown(f'<div class="metric-card"><div class="metric-value metric-accent">{X_all.shape[0]:,}</div><div class="metric-label">CWRU 总样本</div></div>', unsafe_allow_html=True)
